...
// Youtube 정보가 있을 경우에만 추가
    ...
    public List<Work> recommendRelatedWorks(Work searchedWork) {
        ...
        Author author = searchedWork.getAuthor();

        List<Work> recommendedWorks = new ArrayList<>();

        // 동일한 작가일 때, 같은 카테고리를 가진 작품 추천
        if (author != null) {
            List<Work> worksSameAuthor = workRepository.findByAuthorAndCategoriesInOrderByNamе(author, categories);
            recommendedWorks.addAll(worksSameAuthor);
        }

        // 다른 작가일 때, 같은 카테고리를 가진 작품 추천
        List<Work> worksDifferentAuthor = workRepository.findByAuthorNotAndCategoriesInOrderByNamе(author, categories);
        recommendedWorks.addAll(worksDifferentAuthor);
