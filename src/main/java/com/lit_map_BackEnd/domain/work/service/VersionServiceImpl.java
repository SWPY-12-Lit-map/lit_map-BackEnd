...
public class VersionServiceImpl implements VersionService{
  ...
  //  private final MailService mailService;
    ...
    //추가
  //  @Override
    public void confirmVersion(Long versionId, String approvalStatus) {
        Version version = versionRepository.findById(versionId)
                .orElseThrow(() -> new BusinessExceptionHandler(ErrorCode.VERSION_NOT_FOUND));


        if (Objects.equals(approvalStatus, "Y")) {
            version.confirmSetting(Confirm.COMPLETE); //승인
            approveMail(versionId); //승인 완료 메일
            System.out.println("approvalStatus : "+ approvalStatus);
        }

        if (Objects.equals(approvalStatus, "N")) {
            version.confirmSetting(Confirm.LOAD); //반려
            declineMail(versionId, "N"); //승인 반려 메일
        }
    ...
    public MailWorkDto sendMailWithTemplate(Long versionId, String subject, String content) {
        ...
        // Return DTO containing email and work
    ...
    public void approveMail(Long versionId) {
        ...
        System.out.println("이전 버전: " + version.getConfirm());
        //승인
        version.confirmSetting(Confirm.COMPLETE);
        System.out.println("Confirm value set to: " + version.getConfirm());
        versionRepository.save(version);
        System.out.println("Confirm value in database: " + versionRepository.findById(versionId).get().getConfirm());
